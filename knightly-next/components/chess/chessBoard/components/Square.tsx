...
export function Square({
  ...
  function handleDrop(item: BoardPiece | SparePiece) {
    ...
    // Explicitly cast to BoardPiece
    const boardItem = item as BoardPiece;
    if (onPromotionCheck(boardItem.square, square, boardItem.piece)) {
      if (autoPromoteToQueen) {
        handleSetPosition(
          boardItem.square,
          square,
          boardItem.piece[0] === "w" ? "wQ" : "bQ"
        );
      } else {
        setPromoteFromSquare(boardItem.square);
        setPromoteToSquare(square);
        setShowPromoteDialog(true);
      }
    } else {
      handleSetPosition(boardItem.square, square, boardItem.piece, true);
    }
  }
